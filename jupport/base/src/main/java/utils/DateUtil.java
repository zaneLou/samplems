...
public class DateUtil {  
    ...
    public static String fixMonths(String dateString)
	{
		for (int i = 0; i < months.length; i++) {
			dateString = dateString.replace(months[i], (i+1) + "");
		}
        return dateString;
	}
    
    
	public static Date getMondayForThisWeek(Date date)
	{
		Calendar cal =	Calendar.getInstance();
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
        cal.set(Calendar.DAY_OF_WEEK, Calendar.MONDAY); //获取本周一的日期
        return StringToDate(DateToString(cal.getTime(), DateStyle.YYYY_MM_DD), DateStyle.YYYY_MM_DD);
	}
	...
	{
		return StringToDate(DateToString(date, DateStyle.YYYY_MM_DD_HH), DateStyle.YYYY_MM_DD_HH);
	}
    ...
    {
    	return getYear(date) == getYear(otherDate) && getMonth(date) == getMonth(otherDate) && getDay(date) == getDay(otherDate);
    }
